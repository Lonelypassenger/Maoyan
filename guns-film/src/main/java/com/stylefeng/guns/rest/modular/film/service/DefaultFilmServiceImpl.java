 ...
 * @AUTHOR :yuankejia
 * @DESCRIPTION:
 * @DATE:CRETED: IN 15:00 2019/10/25
 * @MODIFY:
 */
@Component
...
public class DefaultFilmServiceImpl implements FilmServiceApi {
    ...
    /**
     * 这个函数的主要作用就是将daoj接口获取到的MoocFilmT列表对象转为我们自定义的FilmInfo列表对象。
     * @param moocFilms
     * @return
     */
    ...
    public FilmVO getHotFilms(boolean isLimit, int nums, int nowPage, int sortId, int sourceId, int yearId, int catId) {
        ...
        // 热映影片的限制条件
        EntityWrapper<MoocFilmT> entityWrapper = new EntityWrapper<>();//用于获取影片
        entityWrapper.eq("film_status","1");//设置获取的影片的状态为1，说明为热映
        // 判断是否是首页需要的内容
        if(isLimit){
            // 如果是，则限制条数、限制内容为热映影片
